...
class MemorySystem:
    ...
    def get_expert_recommendations(self, user_id: str, topic: str = None) -> List[str]:
        ...
        for expert, interaction in memory["expert_interactions"].items():
            ...
            # 向后兼容性处理
            positive = interaction.get("positive_feedback", 0)
            negative = interaction.get("negative_feedback", 0)
    ...
    def get_memory_context(self, user_id: str) -> str:
        ...
        if recent_conversations:
            topics = [conv.get("question", conv.get("topic", "未知话题")) for conv in recent_conversations]
